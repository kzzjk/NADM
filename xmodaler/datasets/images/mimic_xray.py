...
__all__ = ["MIMICxrayDataset"]


@DATASETS_REGISTRY.register()
class MIMICxrayDataset:
    @configurable
    def __init__(
            self,
            stage: str,
    ...
    def from_config(cls, cfg, stage: str = "train"):
        ann_files = {
            "train": os.path.join(cfg.DATALOADER.ANNO_FOLDER, "mimic_caption_anno_train.pkl"),
            "val": os.path.join(cfg.DATALOADER.ANNO_FOLDER, "mimic_caption_anno_val.pkl"),
            "test": os.path.join(cfg.DATALOADER.ANNO_FOLDER, "mimic_caption_anno_test.pkl")
        }
        ret = {
            "stage": stage,
            ...
            "class_nun": cfg.MODEL.VOCAB_SIZE,
    ...
    def __call__(self, dataset_dict):
        ...
        if len(self.image_file) > 0:
            image_path = dataset_dict['image_path'][0]
            image = Image.open(os.path.join(self.image_file, image_path)).convert('RGB')
        ...
        tokens_weight = [dataset_dict['tokens_weight'].astype(np.int64)]
        ret.update({
            ...
            kfg.TOKEN_WEIGHT: tokens_weight
        })
