...
class TransformerEncoderDecoder(BaseEncoderDecoder):
    ...
    def _forward(self, batched_inputs):
        ...
        if kfg.IMAGE_FEATS in inputs:
            attfeat = self.visual_backbone(batched_inputs)
            inputs.update(attfeat)
        ...
        #inputs.update({kfg.EMBED_WEIGHT: self.predictor.decoder.weight})
