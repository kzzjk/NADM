...
class BaseEncoderDecoder(nn.Module, metaclass=ABCMeta):
    ...
    def from_config(cls, cfg):
        return {
            ...
            "visual_backbone": build_backbone(cfg, cfg.MODEL.BACKBONE.NAME),
    ...
    def preprocess_batch(self, batched_inputs):
        ...
        if kfg.IMAGE_FEATS in batched_inputs[0]:
            image = [x[kfg.IMAGE_FEATS] for x in batched_inputs]
            image = pad_tensor(image, padding_value=0, use_mask=False)
            ret = {kfg.IMAGE_FEATS: image}
        ...
        if kfg.G_TOKENS_IDS in batched_inputs[0]:
            ...
            g_tokens_ids, tmasks = pad_tensor(g_tokens_ids, padding_value=0, use_mask=True, type='t')
        ...
        ####################################### XRAY #######################################
        if kfg.NORMAL_LABEL in batched_inputs[0]:
            normal_labels = [x[kfg.NORMAL_LABEL] for x in batched_inputs]
            normal_labels = pad_tensor(normal_labels, padding_value=-1, use_mask=False)
            ret.update({kfg.NORMAL_LABEL: normal_labels})

        if kfg.ABNORMAL_LABEL in batched_inputs[0]:
            abnormal_labels = [x[kfg.ABNORMAL_LABEL] for x in batched_inputs]
            abnormal_labels = pad_tensor(abnormal_labels, padding_value=-1, use_mask=False)
            ret.update({kfg.ABNORMAL_LABEL: abnormal_labels})
        if kfg.TOKEN_WEIGHT in batched_inputs[0]:
            token_weight = [x[kfg.TOKEN_WEIGHT] for x in batched_inputs]
            token_weight = pad_tensor(token_weight, padding_value=-1, use_mask=False)
            ret.update({kfg.TOKEN_WEIGHT: token_weight})
        ######################################################################################
        # if kfg.SEQ_PER_SAMPLE in batched_inputs[0]:
        #     ret.update({kfg.SEQ_PER_SAMPLE: batched_inputs[0][kfg.SEQ_PER_SAMPLE]})
        if kfg.SEQ_PER_SAMPLE in batched_inputs[0]:
        ...
        dict_to_cuda(ret)
        if kfg.STAGE in batched_inputs[0]:
            ret.update({kfg.STAGE: batched_inputs[0][kfg.STAGE]})
