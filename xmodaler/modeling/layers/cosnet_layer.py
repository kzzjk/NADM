...
class COSJointAttention(nn.Module):
    ...
    def forward(
        ...
        # vl_outputs = self.vl_proj(torch.cat([v_outputs, l_outputs], dim=-1))
        # ol_outputs = self.ol_proj(torch.cat([o_outputs, l_outputs], dim=-1))
        # gate = self.gate_attn(torch.cat([vl_outputs, ol_outputs], dim=-1))
        # gate = self.gate(gate)
        # outputs = gate * vl_outputs + (1 - gate) * ol_outputs
