...
class EvalHook(HookBase):
    ...
    def __init__(self, eval_period, eval_start, eval_function, iters_per_epoch, stage, multi_gpu_eval):
        ...
        self.best_score = {'epoch':0,'Bleu_1':0,'Bleu_2':0,'Bleu_3':0,'Bleu_4':0,'METEOR':0,'ROUGE_L':0,'CIDEr':0}
        ...
        if comm.is_main_process():
            if results:
                ...
                if self._stage=='test':
                    count = 0
                    for k, v in flattened_results.items():
                        if v > self.best_score[k]:
                            count = count+1
                    if count>=4 :
                        self.trainer.save_best = 1
                        self.best_score['epoch'] = epoch
                        for k, v in flattened_results.items():
                            self.best_score[k] = v
                if self.trainer.iter == self.trainer.max_iter and self._stage=='test':
                    logger.info(
                        '############################## best_scores ##############################')
