...
class DefaultTrainer(TrainerBase):
        ...
        with torch.no_grad():
            for data in tqdm.tqdm(test_data_loader):
                ...
                for id, output in zip(ids, outputs):
                    results.append({cfg.INFERENCE.ID_KEY: str(id), cfg.INFERENCE.VALUE: output})
    ...
    def run_step(self):
        ...
        # print("Number of parameters: %.2f" % (total/1e6))
        # if self.iter / self.iters_per_epoch > 3:
        #     for para in self.model.named_parameters():
        #         para[1].requires_grad = True
        #     for para in self.model.token_embed.named_parameters():
        #         para[1].requires_grad = False
        #     for para in self.model.visual_embed.named_parameters():
        #         para[1].requires_grad = False
        #     for para in self.model.encoder.named_parameters():
        #         if para[0] in ['embeddings.0.weight', 'embeddings.1.weight', 'embeddings.1.bias']:
        #             para[1].requires_grad = True
        #         else:
        #             para[1].requires_grad = False
        # else:
        #     for para in self.model.named_parameters():
        #         para[1].requires_grad = True
        #     for para in self.model.predictor.named_parameters():
        #         para[1].requires_grad = False
        #     for para in self.model.decoder.named_parameters():
        #         para[1].requires_grad = False
        #     for para in self.model.encoder.named_parameters():
        #         if para[0] in ['embeddings.0.weight', 'embeddings.1.weight', 'embeddings.1.bias']:
        #             para[1].requires_grad = False
