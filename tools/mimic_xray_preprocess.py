...
def build_vocab(cap_path, threshold):
    ...
    for k, v in captions.items():
        ...
        for i in range(len(examples)):
            name = examples[i]['id']
            report = clean_report_mimic_cxr(examples[i]['report'])
...
def cap2idx(args, vocab):
    ...
    pkl.dump(train_gt, open(os.path.join(args.save_path, "mimic_train_gts.pkl"), "wb"))
    for split in datalist:
        pkl.dump(datalist[split], open(os.path.join(args.save_path, "mimic_caption_anno_{}.pkl".format(split)), "wb"))
...
def main(args):
    ...
    with open(os.path.join(args.save_path, "mimic_vocabulary.txt"), "w") as fout:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_path', type=str, default='/home/zgs/X-ray/mimic_cxr/new_data/',
                        help='path for saving vocabulary wrapper')
